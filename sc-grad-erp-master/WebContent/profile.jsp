...
<%
	for(EmployeeDTO dto: list){
		if(dto.getE_NAME().equals(session.getAttribute("loginUser"))){
			nowUser=dto;
		}
	}
}
...
<div class="container-fluid mt-2 mb-5">
	...
	<div class="row mt-4">
		<!-- 거래처관리 폼 시작 -->
		<div class="col-md-8 mt-6" style="font-size: 20px;">
			<h1><%=session.getAttribute("loginUser")%>님 안녕하세요! SemiColon-ERP 시스템입니다.</h1>
			<p>아이디: <%=nowUser.getEID() %></p>
			<p>이름: <%=nowUser.getE_NAME() %></p>
			<p>부서: <%=nowUser.getDEPARTMENT() %></p>
			<p>직급: <%=nowUser.getE_POSITION() %></p>
			<p>입사일: <%=nowUser.getJOIN_DATE() %></p>
		</div>
