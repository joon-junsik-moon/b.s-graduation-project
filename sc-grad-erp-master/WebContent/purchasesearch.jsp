...
<%
	...
	String smn=request.getParameter("smName");
...
<div class="container-fluid mt-2 mb-5">
	...
	<div class="row mt-4">
		...
		<div class="col-md-10">
			<!-- 검색옵션 -->
			<form action='purchasesearch.jsp'>
			<h3>검색</h3>
			<div class="form-row">
				<div class="form-group col-sm-4">
					<label>자재명</label>
					<input type="text" name="smName" class="form-control form-control-sm" id="smName" placeholder="자재 검색">    
				</div>
				<div class="form-group col-sm-4">
					<label>거래처</label>
			...
			int i=1, total=0;
			 for(PurchaseDTO dto:list){
					if(dto.getMATERIALS_ID().contains(smn) && dto.getSELLER_BID().contains(scn) && dto.getPURCHASE_EID().contains(sen) && ((std.compareTo(dto.getD_DATE())<=0)||std.equals(""))&&((edd.compareTo(dto.getD_DATE())>=0)||edd.equals(""))){
				 ...
				 %>
					    <tr align="center">
					      ...
					      <td><button class="btn btn-default btn-sm" name="button" onclick="idDelete('<%=dto.getPURCHASE_CODE()%>')">삭제</button></td>
					      <td><button class="btn btn-default btn-sm" name="button" onclick="idUpdate('<%=dto.getPURCHASE_CODE()%>')">수정</button></td>
					    </tr>
				 <% }
				 }
				 %>
