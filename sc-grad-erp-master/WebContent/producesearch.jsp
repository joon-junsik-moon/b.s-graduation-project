...
<div class="container-fluid mt-2 mb-5">
	...
	<div class="row mt-4">
		...
		<div class="col-md-10">
			<!-- 검색옵션 -->
			<form action='producesearch.jsp'>
			<h3>검색</h3>
			<div class="form-row">
				<div class="form-group col-sm-6">
					<label>제품명</label>
					<input type="text" name="spName" class="form-control form-control-sm" id="spName" placeholder="생산목록 검색">   
				</div>
				<div class="form-group col-sm-6">
					<label>담당자</label>
			...
			for(ProductionManagementDTO dto:list){
				if(dto.getPRODUCTION_CODE().contains(spn) && dto.getPRODUCTION_EID().contains(sen) && ((std.compareTo(dto.getP_DATE())<=0)||std.equals(""))&&((edd.compareTo(dto.getP_DATE())>=0)||edd.equals(""))){
			 %>
			    <tr align="center">
			      <td><%=i++ %></td>
			      ...
			      <td><button class="btn btn-default btn-sm" name="button" onclick="idDelete('<%=dto.getPRODUCTION_CODE()%>')">삭제</button></td>
			      <td><button class="btn btn-default btn-sm" name="button" onclick="idUpdate('<%=dto.getPRODUCTION_CODE()%>')">수정</button></td>
			    </tr>
			 <% }
			}
			 %>
