...
<%
	...
	String spn=request.getParameter("spName");
	String scn=request.getParameter("scName");
	String sen=request.getParameter("seName");
	String std=request.getParameter("startDate");
	String edd=request.getParameter("endDate");
%>
<!-- 
<script>
	alert('빈 값이 있습니다.');
...
<div class="container-fluid mt-2 mb-5">
	...
	<div class="row mt-4">
			...
			<br>
				...
				<hr>
				  ...
				  <div class="form-row">
				    <div class="form-group col-sm-6">
				    	...
				    	<select name="productlist" class="form-control form-control-sm" id="productlist">
							<%
				    		ArrayList<ProductDTO> m=s.ProductList(); 
				    ...
				    <div class="form-group col-sm-6">
				    	...
				    	<select name="clientlist" class="form-control form-control-sm" id="clientlist">
							<%
				    		ArrayList<AccountListDTO> a=s.AccountListList(); 
				  ...
				  <div class="form-row">
				    <div class="form-group col-sm-6">
				    	<label>판매수량</label>
				    	<input type="number" name="amount" class="form-control form-control-sm" id="amount" placeholder="판매수량">
				    </div>
				    <div class="form-group col-sm-6">
				    	<label>판매단가</label>
				    	<input type="number" name="unitprice" class="form-control form-control-sm" id="unitprice" placeholder="판매단가">
				   	</div>
				  </div>
				  ...
				  <div class="form-row">
				    	...
				    	<select name="employeelist" class="form-control form-control-sm" id="employeelist">
							<%
				    		ArrayList<EmployeeDTO> e=s.EmployeeList(); 
		...
		<div class="col-md-10">
			<!-- 검색옵션 -->
			<form action='salesearch.jsp'>
			<h3>검색</h3>
			<div class="form-row">
				<div class="form-group col-sm-4">
					<label>제품명</label>
					<input type="text" name="spName" class="form-control form-control-sm" id="spName" placeholder="제품 검색">    
				</div>
				<div class="form-group col-sm-4">
					<label>거래처</label>
					<input type="text" name="scName" class="form-control form-control-sm" id="scName" placeholder="거래처 검색">
				</div>
				<div class="form-group col-sm-4">
					<label>담당자</label>
					<input type="text" name="seName" class="form-control form-control-sm" id="seName" placeholder="사원 검색">
				</div>
			</div>
			<label>거래일</label>
			<div class="form-row">
			...
			for(SaleDTO dto:list){
				if(dto.getPRODUCT_ID().contains(spn) && dto.getVENDOR_BID().contains(scn) && dto.getSALE_EID().contains(sen) && ((std.compareTo(dto.getD_DATE())<=0)||std.equals(""))&&((edd.compareTo(dto.getD_DATE())>=0)||edd.equals(""))){
					total+=dto.getS_PRICE();
					 %>
						    <tr align="center">
						      <td><%=i++ %></td>
							  ...
							  <td><button class="btn btn-default btn-sm" name="button" onclick="idDelete('<%=dto.getSALE_CODE()%>')">삭제</button></td>
							  <td><button class="btn btn-default btn-sm" name="button" onclick="idUpdate('<%=dto.getSALE_CODE()%>')">수정</button></td>
			...
			<p align="right" style="font-size:17px; font-weight: bold;">판매 금액 총계: <%=fomatter.format(total) %>원 　　</p>
			<!-- 데이터 열람테이블 끝 -->
		</div>
	</div>
</div>
